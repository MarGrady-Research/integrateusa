...
export default function Control ({radio, handleRadio, handleVisibility, handleBounds}) {
    ...
    const legend = () => race.map(el => {
        return(
            <div key={el.race} className='flex items-center'> 
                <span className='w-4 h-4 rounded-sm mr-2 p-1' key={el.color} style={{'backgroundColor': el.color}}></span>
